...
def get_admin_dashboard_stats(
    ...
    total_penelitian = db.query(Research).count()
...
def create_nested_document(
    ...
    if payload.jenis_dokumen_id in [2, 3]:
        ...
        if payload.jenis_dokumen_id == 2:
            copy_source_id = proposal.id

        if payload.jenis_dokumen_id == 3:
            progress = (
                db.query(Document)
                .filter(
                    Document.penelitian_id == research.id,
    ...
    if copy_source_id and payload.jenis_dokumen_id in [2, 3]:
        copy_document_data(db, copy_source_id, document.id, template.id)
...
def create_final_report_document(
    ...
    copy_document_data(db, progress.id, document.id, template.id)
...
def copy_document_data(
    ...
    for b in db.query(Budget).filter(Budget.dokumen_id == source_dokumen_id).all():
        db.add(Budget(
            dokumen_id=target_dokumen_id,
            jenis_pembelanjaan=b.jenis_pembelanjaan,
            item=b.item,
            satuan=b.satuan,
            volume=b.volume,
            biaya_satuan=b.biaya_satuan,
            total=b.total
        ))
    ...
    for o in db.query(Output).filter(Output.dokumen_id == source_dokumen_id).all():
        db.add(Output(
            dokumen_id=target_dokumen_id,
            kategori_luaran=o.kategori_luaran,
            jenis_luaran=o.jenis_luaran,
            status_target=o.status_target,
            keterangan=o.keterangan,
            tahun_luaran=o.tahun_luaran
        ))
    ...
    if source_doc and target_doc:
        source_fields = {
            f.id: f.nama_field
            for f in db.query(TemplateField)
            .filter(TemplateField.template_id == source_doc.template_id)
            .all()
        }
        ...
        target_fields = {
            f.nama_field: f.id
            for f in db.query(TemplateField)
            .filter(TemplateField.template_id == target_template_id)
            .all()
        }

        source_contents = (
            db.query(DocumentContent)
            .filter(DocumentContent.dokumen_id == source_dokumen_id)
            .all()
        )
        ...
        for sc in source_contents:
            ...
            if nama_field and nama_field in target_fields:
                db.add(DocumentContent(
                    dokumen_id=target_dokumen_id,
                    template_field_id=target_fields[nama_field],
                    isi=sc.isi
                ))
        ...
        if target_doc.jenis_dokumen_id == 3 and "prakata" in target_fields and "prakata" not in copied_field_names:
            research = db.query(Research).filter(Research.id == target_doc.penelitian_id).first()
            research_judul = research.judul_penelitian if research else "-"
            ...
            db.add(DocumentContent(
                dokumen_id=target_dokumen_id,
                template_field_id=target_fields["prakata"],
                isi=default_prakata
            ))
