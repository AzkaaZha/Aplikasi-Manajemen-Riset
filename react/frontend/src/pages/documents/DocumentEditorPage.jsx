...
const DocumentEditorPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        if (isCreateMode) {
          ...
          // Load Identitas Fields from Proposal for edit mode display
          if (proposalRes) {
            const getProposalFieldVal = (name) => {
              const f = proposalRes.fields?.find(field => field.nama_field === name);
              return f ? f.isi : "";
            ...
            flatData.judul_penelitian = flatData.judul_penelitian || getProposalFieldVal("judul_penelitian") || proposalRes.judul || "-";
            flatData.bidang_fokus_rirn = flatData.bidang_fokus_rirn || getProposalFieldVal("bidang_fokus_rirn") || "-";
            flatData.tema_penelitian = flatData.tema_penelitian || getProposalFieldVal("tema_penelitian") || "-";
            flatData.topik_penelitian = flatData.topik_penelitian || getProposalFieldVal("topik_penelitian") || "-";
            flatData.rumpun_bidang_ilmu = flatData.rumpun_bidang_ilmu || getProposalFieldVal("rumpun_bidang_ilmu") || "-";
            flatData.target_akhir_tkt = flatData.target_akhir_tkt || getProposalFieldVal("target_akhir_tkt") || "-";
            flatData.lama_penelitian = flatData.lama_penelitian || getProposalFieldVal("lama_penelitian") || "-";
            flatData.kata_kunci = flatData.kata_kunci || getProposalFieldVal("kata_kunci") || "-";
  ...
  const renderProgressStep = () => {
    switch (currentStep) {
      ...
      case 6: return <ProgressLampiranStep data={formData} documentId={activeDocumentId} refreshData={() => getDocumentFiles(activeDocumentId).then(res => setFormData(p => ({...p, lampiranFiles: res})))} />;
      case 7: return <ProgressSelesaiStep onExport={handleExportPdf} isExporting={pdfExporting} />;
      default: return null;
    }
  };
  ...
  const renderFinalStep = () => {
    switch (currentStep) {
      ...
      case 8: return <FinalLampiranStep data={formData} documentId={activeDocumentId} refreshData={() => getDocumentFiles(activeDocumentId).then(res => setFormData(p => ({...p, lampiranFiles: res})))} />;
      case 9: return <FinalSelesaiStep onExport={handleExportPdf} isExporting={pdfExporting} />;
      default: return null;
    }
  };
  ...
  const renderProposalStep = () => {
    switch (currentStep) {
      ...
      case 7: return <ProposalLampiranStep data={formData} documentId={activeDocumentId} refreshData={() => getDocumentFiles(activeDocumentId).then(res => setFormData(p => ({...p, lampiranFiles: res})))} />;
      case 8: return <SelesaiStep onExport={handleExportPdf} isExporting={pdfExporting} />;
      default: return null;
    }
  };
